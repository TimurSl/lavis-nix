...
Help v2 использует статические поля v1 для built-in cards. Активные внешние descriptors и command refs поступают из external runtime отдельным каналом и не добавляются в `MODULE_SPECS`/`COMMAND_SPECS`.

Порядок тем справки: встроенная каноническая команда → активная внешняя namespaced-команда → псевдоним → встроенный модуль → активный внешний модуль → неизвестная тема.
